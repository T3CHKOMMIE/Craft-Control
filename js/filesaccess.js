...
var persistentSettingLockFile = System.Gadget.path + "\\persistentSettingsLockFile.txt";
var persistentSettingFile = System.Gadget.path + "\\serverfiles\\servers.txt";
var persistentSettingTempFile = System.Gadget.path + "\\persistentSettings.tmp.txt";
//
...
var persistentSettingRegExpFormatString = persistentSettingDelimiter + "(..*)" + persistentSettingDelimiter + "(.*)";
...
function setPersistentSettingInternal(name, value)
{
	...
	try
	{
		...
		catch (e) // assume file not found - don't know a language-neutral way to detect other exceptions
		{
			...
			try
			{
				newFile.WriteLine(persistentSettingDelimiter + name + persistentSettingDelimiter + value);
			}
			finally
			{
				newFile.Close();
		...
		try
		{
			...
			try
			{
				while (!oldSettings.AtEndOfStream)
				{
					...
					{
						newSettings.WriteLine(persistentSettingDelimiter + name + persistentSettingDelimiter + value);
						settingReplaced = true;
					}
					else
					{
						newSettings.WriteLine(persistentSettingDelimiter + nameFromFile + persistentSettingDelimiter + valueFromFile);
					}
				}
				if (!settingReplaced)
				{
